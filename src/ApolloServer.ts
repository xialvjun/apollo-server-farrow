...
const fileUploadMiddleware = (uploadsConfig: FileUploadOptions, server: ApolloServerBase): HttpMiddleware => async (
  ...
  if (typeis(useReq(), ['multipart/form-data'])) {
    try {
      const body = await processFileUploads?.(useReq(), useRes(), uploadsConfig)
...
export class ApolloServer extends ApolloServerBase {
  ...
  async createGraphQLServerOptions(req: RequestInfo): Promise<GraphQLOptions> {
  ...
  public getMiddleware({
    ...
    disableHealthCheck,
    onHealthCheck,
  }: GetMiddlewareOptions = {}): HttpMiddleware {
    ...
    if (!disableHealthCheck) {
      middlewares.push(
        middlewareFromPath('/.well-known/apollo/server-health', (req, next) => {
          let res = Response
